...
class QTagParserCustom {
    ...
    private static LineBufferReaderCustom sStatsReader = new LineBufferReaderCustom();
    private static ByteArrayScannerCustom sScanner = new ByteArrayScannerCustom();

    @Nullable
    public static QTagParserCustom sInstance;

    @Nonnull
    public static synchronized QTagParserCustom getInstance() {
        if (sInstance == null) {
            sInstance = new QTagParserCustom(QTAGUID_UID_STATS);
        }
        return sInstance;
    }
    ...
    // @VisibleForTesting
    public QTagParserCustom(String path) {
        mPath = path;
    }
